...
from src.logtrucks.models.common import *
from src.logtrucks.yolo.utils.autoanchor import check_anchor_order
from src.logtrucks.yolo.utils.general import make_divisible, check_file, set_logging
from src.logtrucks.yolo.utils.torch_utils import time_synchronized, fuse_conv_and_bn, \
    model_info, scale_img, initialize_weights, \
    select_device, copy_attr
from src.logtrucks.yolo.utils.loss import SigmoidBin
    ...
    def fuse(self):
        ...
        for i in range(len(self.m)):
            ...
            self.m[i].bias += torch.matmul(self.m[i].weight.reshape(c1, c2),
                                           self.ia[i].implicit.reshape(c2_,
                                                                       c1_)).squeeze(
                1)
...
class IKeypoint(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), nkpt=17, ch=(), inplace=True,
            ...
            if self.dw_conv_kpt:  # keypoint head is slightly more complex
                self.m_kpt = nn.ModuleList(
                    nn.Sequential(DWConv(x, x, k=3), Conv(x, x),
                                  ...
                                  DWConv(x, x, k=3), Conv(x, x),
                                  DWConv(x, x, k=3),
                                  nn.Conv2d(x, self.no_kpt * self.na, 1)) for x
                    in ch)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            if self.nkpt is None or self.nkpt == 0:
                ...
                x[i] = torch.cat((self.im[i](self.m[i](self.ia[i](x[i]))),
                                  self.m_kpt[i](x[i])), axis=1)
            ...
            if not self.training:  # inference
                ...
                if self.inplace:
                    xy = (y[..., 0:2] * 2. - 0.5 + self.grid[i]) * self.stride[
                        i]  # xy
                    wh = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i].view(1,
                                                                           self.na,
                                                                           1, 1,
                                                                           2)  # wh
                    if self.nkpt != 0:
                        x_kpt[..., 0::3] = (x_kpt[...,
                                            ::3] * 2. - 0.5 + kpt_grid_x.repeat(
                            1, 1, 1, 1, 17)) * self.stride[i]  # xy
                        x_kpt[..., 1::3] = (x_kpt[...,
                                            1::3] * 2. - 0.5 + kpt_grid_y.repeat(
                            1, 1, 1, 1, 17)) * self.stride[i]  # xy
                    ...
                    if self.nkpt != 0:
                        y[..., 6:] = (y[..., 6:] * 2. - 0.5 + self.grid[
                            i].repeat((1, 1, 1, 1, self.nkpt))) * self.stride[
                                         i]  # xy
...
class IAuxDetect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        self.register_buffer('anchor_grid', a.clone().view(self.nl, 1, -1, 1, 1,
                                                           2))  # shape(nl,1,na,1,1,2)
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in
                               ch[:self.nl])  # output conv
        self.m2 = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in
                                ch[self.nl:])  # output conv
...
class IBin(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(),
        ...
        self.w_bin_sigmoid = SigmoidBin(bin_count=self.bin_count, min=0.0,
                                        max=4.0)
        self.h_bin_sigmoid = SigmoidBin(bin_count=self.bin_count, min=0.0,
                                        max=4.0)
        # classes, x,y,obj
        self.no = nc + 3 + \
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                pw = self.w_bin_sigmoid.forward(y[..., 2:24]) * \
                     self.anchor_grid[i][..., 0]
                ph = self.h_bin_sigmoid.forward(y[..., 24:46]) * \
...
class Model(nn.Module):
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if self.traced:
                if isinstance(m, Detect) or isinstance(m,
                                                       IDetect) or isinstance(m,
            ...
            if profile:
                ...
                o = \
                thop.profile(m, inputs=(x.copy() if c else x,), verbose=False)[
    ...
    def _initialize_biases(self,
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            b.data[:, 5:] += math.log(
                0.6 / (m.nc - 0.99)) if cf is None else torch.log(
                cf / cf.sum())  # cls
            mi.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)

    def _initialize_aux_biases(self,
                               cf=None):  # initialize biases into Detect(), cf is class frequency
        # https://arxiv.org/abs/1708.02002 section 3.3
        # cf = torch.bincount(torch.tensor(np.concatenate(dataset.labels, 0)[:, 0]).long(), minlength=nc) + 1.
        m = self.model[-1]  # Detect() module
    ...
    def _initialize_biases_bin(self,
                               cf=None):  # initialize biases into Detect(), cf is class frequency
        # https://arxiv.org/abs/1708.02002 section 3.3
        # cf = torch.bincount(torch.tensor(np.concatenate(dataset.labels, 0)[:, 0]).long(), minlength=nc) + 1.
    ...
    def _initialize_biases_kpt(self,
                               cf=None):  # initialize biases into Detect(), cf is class frequency
        # https://arxiv.org/abs/1708.02002 section 3.3
        # cf = torch.bincount(torch.tensor(np.concatenate(dataset.labels, 0)[:, 0]).long(), minlength=nc) + 1.
        m = self.model[-1]  # Detect() module
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv,
                 RepConv, RepConv_OREPA, DownC,
                 SPP, SPPF, SPPCSPC, GhostSPPCSPC, MixConv2d, Focus, Stem,
                 GhostStem, CrossConv,
        ...
        logger.info(
            '%3s%18s%3s%10.0f  %-40s%-30s' % (i, f, n, np, t, args))  # print
