...
class RobustConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=7, s=1, p=None, g=1, act=True,
        ...
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones(
            c2)) if layer_scale_init_value > 0 else None
...
class ResX(Res):
    # ResNet bottleneck
    def __init__(self, c1, c2, shortcut=True, g=32,
                 e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
...
class autoShape(nn.Module):
    ...
    def autoshape(self):
        print(
            'autoShape already enabled, skipping... ')  # model already converted to model.autoshape()
        return self
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        for i, im in enumerate(imgs):
            ...
            if isinstance(im, str):  # filename or uri
                im, f = np.asarray(Image.open(
                    requests.get(im, stream=True).raw if im.startswith(
                        'http') else im)), im
            ...
            im = im[:, :, :3] if im.ndim == 3 else np.tile(im[:, :, None],
                                                           3)  # enforce 3ch input
            s = im.shape[:2]  # HWC
            ...
            imgs[i] = im  # update
        shape1 = [make_divisible(x, int(self.stride.max())) for x in
                  np.stack(shape1, 0).max(0)]  # inference shape
        x = [letterbox(im, new_shape=shape1, auto=False)[0] for im in
             imgs]  # pad
        ...
        with amp.autocast(enabled=p.device.type != 'cpu'):
            ...
            # Post-process
            y = non_max_suppression(y, conf_thres=self.conf, iou_thres=self.iou,
...
class Detections:
    ...
    def __init__(self, imgs, pred, files, times=None, names=None, shape=None):
        ...
        gn = [torch.tensor([*[im.shape[i] for i in [1, 0, 1, 0]], 1., 1.],
                           device=d) for im in imgs]  # normalizations
    ...
    def display(self, pprint=False, show=False, save=False, render=False,
        ...
        for i, (img, pred) in enumerate(zip(self.imgs, self.pred)):
            ...
            img = Image.fromarray(img.astype(np.uint8)) if isinstance(img,
                                                                      np.ndarray) else img  # from np
            if pprint:
                print(str.rstrip(', '))
            if show:
...
def transI_fusebn(kernel, bn):
    gamma = bn.weight
    std = (bn.running_var + bn.eps).sqrt()
    return kernel * ((gamma / std).reshape(-1, 1, 1,
                                           1)), bn.bias - bn.running_mean * gamma / std
...
class OREPA_3x3_RepConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        if internal_channels_1x1_3x3 == in_channels:
            ...
            nn.init.kaiming_uniform_(self.weight_rbr_1x1_kxk_conv1,
                                     a=math.sqrt(1.0))
        self.weight_rbr_1x1_kxk_conv2 = nn.Parameter(torch.Tensor(out_channels,
    ...
    def weight_gen(self):
        ...
        if self.groups > 1:
            ...
            weight_rbr_1x1_kxk_conv1 = weight_rbr_1x1_kxk_conv1.view(g,
                                                                     int(t / g),
                                                                     ig)
            weight_rbr_1x1_kxk_conv2 = weight_rbr_1x1_kxk_conv2.view(g,
                                                                     int(o / g),
                                                                     tg, h, w)
            weight_rbr_1x1_kxk = torch.einsum('gti,gothw->goihw',
                                              weight_rbr_1x1_kxk_conv1,
                                              weight_rbr_1x1_kxk_conv2).view(o,
                                                                             ig,
                                                                             h,
                                                                             w)
        else:
...
class RepConv_OREPA(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1, padding=1, dilation=1, groups=1,
        ...
        if deploy:
            ...
            self.rbr_1x1 = ConvBN(in_channels=self.in_channels,
                                  ...
                                  stride=s, padding=padding_11, groups=groups,
                                  dilation=1)
    ...
    def get_custom_L2(self):
        ...
        t3 = (self.rbr_dense.bn.weight / ((
                                                      self.rbr_dense.bn.running_var + self.rbr_dense.bn.eps).sqrt())).reshape(
            -1, 1, 1, 1).detach()
        t1 = (self.rbr_1x1.bn.weight / ((
                                                    self.rbr_1x1.bn.running_var + self.rbr_1x1.bn.eps).sqrt())).reshape(
            -1, 1, 1, 1).detach()

        l2_loss_circle = (K3 ** 2).sum() - (K3[:, :, 1:2,
                                            1:2] ** 2).sum()  # The L2 loss of the "circle" of weights in 3x3 kernel. Use regular L2 on them.
        eq_kernel = K3[:, :, 1:2,
                    1:2] * t3 + K1 * t1  # The equivalent resultant central point of 3x3 kernel.
...
class SwinTransformerLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        Padding = False
        if min(H_,
               W_) < self.window_size or H_ % self.window_size != 0 or W_ % self.window_size != 0:
            Padding = True
            # print(f'img_size {min(H_, W_)} is less than (or not divided by) window_size {self.window_size}, Padding.')
            pad_r = (
                                self.window_size - W_ % self.window_size) % self.window_size
            pad_b = (
                                self.window_size - H_ % self.window_size) % self.window_size
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, c1, c2, num_heads, num_layers, window_size=8):
        ...
        self.blocks = nn.Sequential(*[
            SwinTransformerLayer(dim=c2, num_heads=num_heads,
...
class WindowAttention_v2(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # cosine attention
        attn = (F.normalize(q, dim=-1) @ F.normalize(k, dim=-1).transpose(-2,
                                                                          -1))
...
class SwinTransformerLayer_v2(nn.Module):
    ...
    def forward(self, x):
        ...
        # partition windows
        x_windows = window_partition_v2(shifted_x,
                                        self.window_size)  # nW*B, window_size, window_size, C
        x_windows = x_windows.view(-1, self.window_size * self.window_size,
                                   C)  # nW*B, window_size*window_size, C
...
class SwinTransformer2Block(nn.Module):
    def __init__(self, c1, c2, num_heads, num_layers, window_size=7):
        ...
        # remove input_resolution
        self.blocks = nn.Sequential(*[
            SwinTransformerLayer_v2(dim=c2, num_heads=num_heads,
                                    window_size=window_size,
                                    shift_size=0 if (
                                                i % 2 == 0) else window_size // 2)
            for i in range(num_layers)])
