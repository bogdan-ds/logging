...
class SigmoidBin(nn.Module):
    ...
    def __init__(self, bin_count=10, min=0.0, max=1.0, reg_scale=2.0,
    ...
    def forward(self, pred):
        ...
        pred_reg = (pred[
                        ..., 0] * self.reg_scale - self.reg_scale / 2.0) * self.step
    ...
    def training_loss(self, pred, target):
        ...
        pred_reg = (pred[
                        ..., 0].sigmoid() * self.reg_scale - self.reg_scale / 2.0) * self.step
        pred_bin = pred[..., 1:(1 + self.bin_count)]
    ...
    def forward(ctx, logits, targets, delta_RS=0.50, eps=1e-10):
        ...
        for ii in order:
            ...
            if FP_num > eps:
                ...
                relevant_bg_grad += (
                            bg_relations * (ranking_error[ii] / FP_num))
...
class aLRPLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, regression_losses, delta=1., eps=1e-5):
        ...
        for ii in order:
            ...
            if FP_num > eps:
                fg_grad[ii] = -(torch.sum(
                    fg_relations * regression_losses) + FP_num) / rank[ii]
...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1,
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ps = pi[
                    b, a, gj, gi]  # prediction subset corresponding to targets
                ...
                # Objectness
                tobj[b, a, gj, gi] = (
                                                 1.0 - self.gr) + self.gr * iou.detach().clamp(
                    0).type(tobj.dtype)  # iou ratio
    ...
    def build_targets(self, p, targets):
        ...
        gain = torch.ones(7,
                          device=targets.device).long()  # normalized to gridspace gain
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                grid = torch.stack([gi, gj], dim=1)
                pxy = (fg_pred[:, :2].sigmoid() * 2. - 0.5 + grid) * \
                      self.stride[i]  # / 8.
            ...
            top_k, _ = torch.topk(pair_wise_iou,
                                  min(10, pair_wise_iou.shape[1]), dim=1)
        ...
        for i in range(nl):
            if matching_targets[i] != []:
                ...
                matching_bs[i] = torch.tensor([], device='cuda:0',
                                              dtype=torch.int64)
                matching_as[i] = torch.tensor([], device='cuda:0',
                                              dtype=torch.int64)
                matching_gjs[i] = torch.tensor([], device='cuda:0',
                                               dtype=torch.int64)
                matching_gis[i] = torch.tensor([], device='cuda:0',
                                               dtype=torch.int64)
                matching_targets[i] = torch.tensor([], device='cuda:0',
                                                   dtype=torch.int64)
                matching_anchs[i] = torch.tensor([], device='cuda:0',
                                                 dtype=torch.int64)
...
class ComputeLossBinOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        wh_bin_sigmoid = SigmoidBin(bin_count=self.bin_count, min=0.0, max=4.0,
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                pbox = torch.cat((px.unsqueeze(1), py.unsqueeze(1),
                                  pw.unsqueeze(1), ph.unsqueeze(1)), 1).to(
                ...
                selected_tcls = targets[i][:, 1].long()
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(ps[:, (1 + obj_idx):], self.cn,
                                        device=device)  # targets
                    t[range(n), selected_tcls] = self.cp
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                pw = self.wh_bin_sigmoid.forward(
                    fg_pred[..., 2:(3 + self.bin_count)].sigmoid()) * anch[i][
                                                                          idx][
                                                                      :, 0] * \
                     self.stride[i]
                ...
                pxywh = torch.cat([pxy, pw.unsqueeze(1), ph.unsqueeze(1)],
                                  dim=-1)
                pxyxy = xywh2xyxy(pxywh)
                pxyxys.append(pxyxy)
...
class ComputeLossAuxOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            if n_aux:
                ps_aux = pi_aux[
                    b_aux, a_aux, gj_aux, gi_aux]  # prediction subset corresponding to targets
                ...
                selected_tbox_aux = targets_aux[i][:, 2:6] * pre_gen_gains_aux[
                    i]
                selected_tbox_aux[:, :2] -= grid_aux
                iou_aux = bbox_iou(pbox_aux.T, selected_tbox_aux,
                                   x1y1x2y2=False,
                                   CIoU=True)  # iou(prediction, target)
                ...
                tobj_aux[b_aux, a_aux, gj_aux, gi_aux] = (
                                                                     1.0 - self.gr) + self.gr * iou_aux.detach().clamp(
                    0).type(tobj_aux.dtype)  # iou ratio

                # Classification
                selected_tcls_aux = targets_aux[i][:, 1].long()
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t_aux = torch.full_like(ps_aux[:, 5:], self.cn,
                                            device=device)  # targets
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            top_k, _ = torch.topk(pair_wise_iou,
                                  min(20, pair_wise_iou.shape[1]), dim=1)
            dynamic_ks = torch.clamp(top_k.sum(1).int(), min=1)
