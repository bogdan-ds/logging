...
from iwpod_net.src.label import Label
from iwpod_net.src.utils import getWH, nms
from iwpod_net.src.projection_utils import getRectPts, find_T_matrix
...
def reconstruct_new(Iorig, I, Y, out_size, threshold=.9):
    net_stride = 2 ** 4
    side = ((
                        208. + 40.) / 2.) / net_stride  # based on rescaling of training data

    Probs = Y[..., 0]
    Affines = Y[...,
              -6:]  # gets the last six coordinates related to the Affine transform
    rx, ry = Y.shape[:2]
    ...
    if len(final_labels):
        ...
        for i, label in enumerate(final_labels):
            ...
            Ilp = cv2.warpPerspective(Iorig, H, out_size, flags=cv2.INTER_CUBIC,
                                      borderValue=.0)
            TLps.append(Ilp)
